...
#AutoIt3Wrapper_Res_Fileversion=2.0.2.1
...
#include <IE.au3>
#endregion - Includes -
Global Const $Buisness = "Computer Stats", $Ver = "2.0.2.1"
...
$Export = GUICtrlCreateMenuItem("Export Info", $ToolsMenu)
...
If @OSVersion = "WIN_VISTA" Or @OSVersion = "WIN_7" Or @OSVersion = "WIN_8" Then
	$EnableDisableUAC = GUICtrlCreateMenuItem("UAC", $ToolsMenu)
	$UAC = RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "EnableLUA")
	If $UAC = 1 Then
		GUICtrlSetData($EnableDisableUAC, "Disable UAC")
	Else
		GUICtrlSetData($EnableDisableUAC, "Enable UAC")
	EndIf
Else
	$EnableDisableUAC = GUICtrlCreateMenuItem("Enable/Disable UAC", $ToolsMenu)
...
For $i = 1 To $aDrive[0]
	...
	If $DriveFree[$i] > 0 Then
		$DriveFreePos[$i] = GUICtrlCreateProgress(125, 25 + (($i - 1) * 15), 125, 15, 0x01)
		GUICtrlSetData($DriveFreePos[$i], $DriveFree[$i])
		GUICtrlCreateLabel(Round((DriveSpaceTotal(StringUpper($aDrive[$i]) & "\") - DriveSpaceFree(StringUpper($aDrive[$i]) & "\")) / 1024, 1) & "/" &  Round((DriveSpaceTotal(StringUpper($aDrive[$i]) & "\") / 1024), 1) & " GB (" & $DriveFree[$i] & "%)", 255, 30 + (($i - 1) * 15))
	EndIf
Next
...
While 1
	...
	Switch $iMsg
		...
		Case $DriveButton[1] To $DriveButton[UBound($DriveButton) - 1]
			DriveInfoGUI(GUICtrlRead($iMsg))
		Case $EnableDisableUAC
			If $UAC = 0 Then
				If MsgBox(4, "Enable UAC", "Are you sure you want to ENABLE User Account Control?") = 6 Then
					RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "EnableLUA", "REG_DWORD", 1)
					If MsgBox(4, "Enable UAC", "Computer must be restarted to finish." & @CRLF & "Make sure to save any documents!" & @CRLF & @CRLF & "Restart now?") = 6 Then
						Shutdown(6)
						Exit
					EndIf
				EndIf
			ElseIf $UAC = 1 Then
				If MsgBox(4, "Disable UAC", "Are you sure you want to DISABLE User Account Control?") = 6 Then
					RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System", "EnableLUA", "REG_DWORD", 0)
					If MsgBox(4, "Disable UAC", "Computer must be restarted to finish." & @CRLF & "Make sure to save any documents!" & @CRLF & @CRLF & "Restart now?") = 6 Then
						Shutdown(6)
						Exit
					EndIf
				EndIf
...
Func DriveInfoGUI($DriveLetter)
	Local $guimsg
	GUISetState(@SW_HIDE, $MainGUI)
	$DriveInfoGUI = GUICreate("Drive Info " & $DriveLetter, 265, 250, -1, -1, 0x00CA0080, $WS_EX_ACCEPTFILES)
	GUISetFont(8, 400, 0, "Lucida Console")
	GUICtrlCreateLabel("Label:", 10, 10, -1, -1)
	$DriveLabel = GUICtrlCreateInput(DriveGetLabel($DriveLetter), 105, 5, 150, 20, 0x0801)
	If DriveGetLabel($DriveLetter) = "" Then GUICtrlSetData($DriveLabel, "No Label Set")
	GUICtrlCreateLabel("Status:", 10, 35, -1, -1)
	$DriveStatus = GUICtrlCreateInput(DriveStatus($DriveLetter), 105, 30, 150, 20, 0x0801)
	GUICtrlCreateLabel("File System:", 10, 60, -1, -1)
	$DriveFileSystem = GUICtrlCreateInput(DriveGetFileSystem($DriveLetter), 105, 55, 150, 20, 0x0801)
	GUICtrlCreateLabel("Type:", 10, 85, -1, -1)
	$DriveInfoType = GUICtrlCreateInput(DriveGetType($DriveLetter) & " Device", 105, 80, 150, 20, 0x0801)
	GUICtrlCreateLabel("Capacity:", 10, 110, -1, -1)
	$DriveSize = GUICtrlCreateInput(Round(DriveSpaceTotal($DriveLetter) / 1024, 1) & " GB", 105, 105, 150, 20, 0x0801)
	GUICtrlCreateLabel("Used Space:", 10, 135, -1, -1)
	$DriveUsedSpace = GUICtrlCreateInput(Round((DriveSpaceTotal($DriveLetter) - DriveSpaceFree($DriveLetter)) / 1024, 1) & " GB", 105, 130, 150, 20, 0x0801)
	GUICtrlCreateLabel("Free Space:", 10, 160, -1, -1)
	$DriveFreeSpace = GUICtrlCreateInput(Round(DriveSpaceFree($DriveLetter) / 1024, 1) & " GB", 105, 155, 150, 20, 0x0801)
	GUICtrlCreateLabel("Serial Number:", 10, 185, -1, -1)
	$DriveSerial = GUICtrlCreateInput(DriveGetSerial($DriveLetter), 105, 180, 150, 20, 0x0801)
	GUICtrlCreateLabel("Temperature:", 10, 210, -1, -1)
	$DriveTemp = GUICtrlCreateInput("", 105, 205, 150, 20, 0x0801)
	...
	EndIf
	$Progress = Round((DriveSpaceTotal($DriveLetter) - DriveSpaceFree($DriveLetter)) / DriveSpaceTotal($DriveLetter) * 100, 0)
	If $Progress > 0 Then
		$DriveProgress = GUICtrlCreateProgress(50, 230, 125, 15, 0x01)
		GUICtrlSetData($DriveProgress, $Progress)
		GUICtrlCreateLabel($Progress & "%", 180, 235)
	EndIf
	If DriveStatus($DriveLetter) = "INVALID" Or $DriveLetter = "" Then
...
Func _Print()
	Local $sText, $fStats, $file, $timestamp
	$timestamp = _DateTimeFormat(_NowCalc(), 2) & " " & _DateTimeFormat(_NowCalc(), 4)
	$timestamp = StringReplace($timestamp, "/", "-")
	$timestamp = StringReplace($timestamp, ":", "")
	$fStats = @ScriptDir & "\Exported Info\" & @ComputerName & " " & $timestamp & ".txt"
	$file = FileOpen($fStats, 10)
	$sText &= "-  -  -  M  a  i  n  -  -  -" & @CRLF & _
	"User Name: " & @TAB & @TAB & @UserName & @CRLF & _
	"Computer Name: " & @TAB & @TAB & @ComputerName & @CRLF & _
	"OS Language: " & @TAB & @TAB & $Language & @CRLF & _
	"Operating System: " & @TAB & $OSVersion & " " & @OSArch & @CRLF & _
	"Service Pack: " & @TAB & @TAB & $OSServicePack & @CRLF & _
	"Manufacturer: " & @TAB & @TAB & StringStripWS($aSystemInfo[1][21], 3) & @CRLF & _
	"Model: " & @TAB & @TAB & @TAB & StringStripWS($aSystemInfo[1][22], 3) & @CRLF & _
	"Serial Number: " & @TAB & @TAB & StringStripWS($BIOS[1][16], 3) & @CRLF & _
	"Display WxH & Refresh: " & @TAB & @DesktopWidth & " x " & @DesktopHeight & " @ " & @DesktopRefresh & " Hz" & @CRLF & _
	"Display Depth: " & @TAB & @TAB & @DesktopDepth & " Bits" & @CRLF & _
	"OS Install Path: " & @TAB & @WindowsDir & "\" & @CRLF & @CRLF & _
	"-  -  -  P  r  o  d  u  c  t    K  e  y  s  -  -  -" & @CRLF & _
	"-- W I N D O W S --" & @CRLF & _
	"Windows Key : " & @TAB & @TAB & _DecodeProductKey("Windows") & @CRLF & _
	"Windows Key 4 : " & @TAB & _DecodeProductKey("Windows_DPid4") & @CRLF & _
	"Windows Default : " & @TAB & _DecodeProductKey("Windows_Def") & @CRLF & _
	"Windows Default 4 : " & @TAB & _DecodeProductKey("Windows_Def_DPid4") & @CRLF & _
	"-- O F F I C E --" & @CRLF & _
	"Office XP Key : " & @TAB & _DecodeProductKey("Office XP") & @CRLF & _
	"Office 2003 Key : " & @TAB & _DecodeProductKey("Office 2003") & @CRLF & _
	"Office 2007 Key : " & @TAB & _DecodeProductKey("Office 2007") & @CRLF & _
	...
	"Office 2010 x64 Key: " & @TAB & _DecodeProductKey("Office 2010 x64") & @CRLF & _
	"Office 2013 x86 Key: " & @TAB & _DecodeProductKey("Office 2013 x86") & @CRLF & _
	"Office 2013 x64 Key: " & @TAB & _DecodeProductKey("Office 2013 x64") & @CRLF & @CRLF & _
	"-  -  -  C  P  U  -  -  -" & @CRLF & _
	"Speed: " & @TAB & @TAB & @TAB & StringFormat('%.2f', ($CPUInfo[1] / 1000)) & " GHz" & @CRLF & _
	"Architecture: " & @TAB & @TAB & @CPUArch & @CRLF & _
	"Name: " & @TAB & @TAB & @TAB & StringStripWS($CPUInfo[2], 7) & @CRLF & _
	"Vendor: " & @TAB & @TAB & $CPUInfo[4] & @CRLF & _
	"Cores: " & @TAB & @TAB & @TAB & $CPUInfo[0] & @CRLF & _
	"Identifier: " & @TAB & @TAB & $CPUInfo[3] & @CRLF & @CRLF & _
	"-  -  -  R  A  M  -  -  -" & @CRLF & _
	"Memory Type: " & @TAB & @TAB & $MemoryType & @CRLF & _
	"Total Physical Memory: " & @TAB & Round($RAMUsage[1] / 1048576, 2) & " GB" & @CRLF & _
	"Total Virtual Memory: " & @TAB & Round($RAMUsage[3] / 1048576, 2) & " GB" & @CRLF & @CRLF & _
	"-  -  -  N  e  t  w  o  r  k  -  -  -" &  @CRLF & _
	"Public IP: " & @TAB & @TAB & $PublicIP & @CRLF & _
	"Local IP: " & @TAB & @TAB & @IPAddress1 & @CRLF & _
	"MAC Address: " & @TAB & @TAB & $IPDetail[1] & @CRLF & _
	"Default Gateway: " & @TAB & $IPDetail[2] & @CRLF & _
	"Network Adapter: " & @TAB & GetWMI($ip) & @CRLF & @CRLF & _
	"-  -  - H  A  R  D    D  R  I  V  E  S  -  -  -" & @CRLF
	For $i = 1 To $aDrive[0]
		$DriveLetter = StringUpper($aDrive[$i]) & "\"
		$sText &= "Drive Letter:" & @TAB & @TAB & $DriveLetter & @CRLF
		If DriveGetLabel($DriveLetter) = "" Then
			$sText &= "Label:"  & @TAB & @TAB & @TAB & "No Label Set" & @CRLF
		Else
			$sText &= "Label:"  & @TAB & @TAB & @TAB & DriveGetLabel($DriveLetter) & @CRLF
		EndIf
		$sText &= "Status:" & @TAB & @TAB & @TAB & DriveStatus($DriveLetter) & @CRLF & _
		"File System:" & @TAB & @TAB & DriveGetFileSystem($DriveLetter) & @CRLF & _
		"Type:" & @TAB & @TAB & @TAB & DriveGetType($DriveLetter) & " Device" & @CRLF & _
		"Capacity:" & @TAB & @TAB & Round(DriveSpaceTotal($DriveLetter) / 1024, 1) & " GB" & @CRLF & _
		"Used Space:" & @TAB & @TAB & Round((DriveSpaceTotal($DriveLetter) - DriveSpaceFree($DriveLetter)) / 1024, 1) & " GB" & @CRLF & _
		"Free Space:" & @TAB & @TAB & Round(DriveSpaceFree($DriveLetter) / 1024, 1) & " GB" & @CRLF & _
		"Serial Number:" & @TAB & @TAB & DriveGetSerial($DriveLetter) & @CRLF & _
		"Temperature:" & @TAB & @TAB
		If DriveGetType($DriveLetter) = "Fixed" Then
			$tmp = _WMI_DriveTemperature()
			$sText &= $tmp & ' °C' & @CRLF & @CRLF
		Else
			$sText &= 'N/A' & @CRLF & @CRLF
		EndIf
	Next
